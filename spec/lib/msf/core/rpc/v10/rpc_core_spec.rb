...
RSpec.describe Msf::RPC::RPC_Core do
  ...
  context 'CONSTANTS' do
    context 'FORMATTED_STATUS_BY_THREAD_STATUS' do
      ...
      it { expect(formatted_status_by_thread_status[false]).to eql 'terminated normally' }
      it { expect(formatted_status_by_thread_status[nil]).to eql 'terminated with exception' }
    end
  end
  ...
  context '#rpc_thread_kill' do
    ...
    context 'with named Thread' do
      ...
      it 'should kill named Thread' do
        expect(framework.threads.list.size).to eq(1)
  ...
  context '#rpc_thread_list' do
    ...
    context 'with spawned thread' do
      ...
      it 'include Metasploit::Framework::Thread#as_json' do
        ...
        json.each do |key, value|
          expect(thread_entry[key]).to eql value
        end
      end

      it 'should include :status' do
        expect(thread_entry[:status]).to eql 'sleep'
      end
    end
    ...
    context 'Thread' do
      ...
      let(:thread) do
        ...
        expect(thread).to receive(:[]).with(:metasploit_framework_thread).and_return(metasploit_framework_thread)
        # thread.stub(status: status)
        expect(thread).to receive(:status).at_least(:once).and_return(status)
      ...
      before(:each) do
        ...
        expect(framework).to receive(:threads).at_least(:once).and_return(thread_manager)
      end
      ...
      context '#status' do
        context 'with aborting' do
          ...
          it { expect(thread_entry[:status]).to eql status }
        end
        ...
        context 'with false' do
          ...
          it { expect(thread_entry[:status]).to eql 'terminated normally' }
        end
        ...
        context 'with nil' do
          ...
          it { expect(thread_entry[:status]).to eql 'terminated with exception' }
        end
        ...
        context 'with sleep' do
          ...
          it { expect(thread_entry[:status]).to eql 'sleep' }
        end
      end
    end
  end
