...
RSpec.describe Metasploit::Framework::Thread::Manager, type: :model do
  subject(:manager) do
  ...
  context '#framework' do
    ...
    it 'should be synchronized' do
      expect(manager).to receive(:synchronize)
      framework
    end
    ...
    it 'should retrieve @framework' do
      ...
      expect(framework).to eql expected_framework
    end
  end
  ...
  context '#framework=' do
    ...
    it 'should set @framework' do
      ...
      expect(manager.instance_variable_get(:@framework)).to eql framework
    end
  end
  ...
  context '#list' do
    ...
    it 'should delegate to #thread_group' do
      expect(manager.send(:thread_group)).to receive(:list)
  ...
  context '#register' do
    ...
    #       require 'pry'
    #       binding.pry
    #       expect(error.metasploit_framework_thread).to eql thread[:metasploit_framework_thread]
    #     end
    #   end
    # end
    ...
    context 'without already registered' do
      context 'with valid attributes' do
        it 'should pass attributes and &block to Metasploit::Framework::Thread.new' do
          expect(Metasploit::Framework::Thread).to receive(:new).with(
        ...
        it 'should set :metasploit_framework_thread thread local variable' do
          ...
          expect(metasploit_framework_thread).to be_a Metasploit::Framework::Thread
        end
        ...
        it 'should set Thread.current.group to #thread_group' do
          ...
          expect(register_thread_group).to eql manager.send(:thread_group)
        end
            ...
            it 'should log and raise exception' do
              thread = Thread.new {
                manager.register(attributes) {
                  ...
                  expect(metasploit_framework_thread).to receive(:log_and_raise).with(exception).and_call_original
            ...
            it 'should clear :metasploit_framework_thread' do
              thread = Thread.new {
                begin
                  ...
                  expect(thread[:metasploit_framework_thread]).to be_nil
                end
              }
          ...
          context 'without exception' do
            it 'should return yieldreturn' do
              thread = Thread.new {
                ...
                expect(manager.register(attributes){yieldreturn}).to eql yieldreturn
              }
            ...
            it 'should clear :metasploit_framework_thread' do
              ...
              thread.join
              expect(thread[:metasploit_framework_thread]).to be nil
            end
    ...
    context 'with registered thread' do
      ...
      context 'in block' do
        it 'should be true' do
          thread = Thread.new {
            manager.register(attributes) {
              expect(manager).to be_registered
            }
          }
  ...
  context '#spawn' do
    ...
    it 'should #register thread' do
      expect(manager).to receive(:register).with(
          hash_including(
              block: block_block,
    ...
    context '&block' do
      ...
      context 'without exception' do
        it 'should return yieldreturn' do
          ...
          expect(thread.value).to eql yieldreturn
        end
        ...
        context 'with database' do
          context 'with active' do
            ...
            before(:each) do
              allow(framework.db).to receive(:active) { true }
            end

            it 'should release connection' do
              # with_established_connection do
              #   expect(ActiveRecord::Base.connection_pool).to receive(:release_connection).and_call_original
              # end
          ...
          context 'without active' do
            before(:each) do
              expect(framework.db).to receive(:active?) { false }
            end

            it 'should not release connection' do
              expect(ActiveRecord::Base).to_not receive(:connection_pool)
        ...
        context 'without database' do
          before(:each) do
            allow(framework).to receive(:db) {nil}
          end
  ...
  context '#thread_group' do
    ...
    context 'with @thread_group' do
      ...
      it 'should return current value' do
        expect(thread_group).to eql expected_thread_group
      end
    end

    context 'without @thread_group' do
      it 'should create a new ThreadGroup' do
        expect(ThreadGroup).to receive(:new)
